...
uint64_t read_Bristol_circuit(string filename, uint32_t &input_num_total, uint32_t &output_num) {
  ...
  for (uint32_t i = input_num_total; i < gate_num + input_num_total; ++i) {
    ...
    if ((current->type == 0) || (current->type == 1) || (current->type == 3) || (current->type == 2 && current->output)) {
    ...
    if (current->type == 3) {
      if ((left_p->type != 2 && right_p->type != 2)) {  // a OR b
        file2 << "0 1 1 1";
      } else if ((left_p->type == 2 && right_p->type != 2)) {  // NOT a OR b
        file2 << "1 1 0 1";
      } else if ((left_p->type != 2 && right_p->type == 2)) {  // a OR NOT b
        file2 << "1 0 1 1";
      } else if ((left_p->type == 2 && right_p->type == 2)) {  //NOT a OR NOT b
        file2 << "1 1 1 0";
      }
      file2 << " ] inputs [ " << left_p->number2 << " " << right_p->number2;
    }
