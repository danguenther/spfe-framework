...
int main(int argc, char *argv[]) {
  ...
  string shdl2;
  shdl2 += filename;
  shdl2 += SHDL_CIRCUIT_FILE_FORMAT;

    // char shdl2[strlen(filename.c_str()) + strlen(SHDL_CIRCUIT_FILE_FORMAT)];
    // strcat(shdl2, filename.c_str());
    // strcat(shdl2, SHDL_CIRCUIT_FILE_FORMAT);

  string shdl;
  shdl += shdl2;
  shdl += SHDL_MOD_CIRCUIT_FILE_FORMAT;

  // char shdl[shdl2.size() + strlen(SHDL_MOD_CIRCUIT_FILE_FORMAT)];
  // strcpy(shdl, shdl2.c_str());
